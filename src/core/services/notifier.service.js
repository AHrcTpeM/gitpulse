...
class NotifierService {
  ...
  async sendConfirmationEmail(email, token) {
    ...
    await mailClient.send({
      to: email,
      subject: '✉️ Confirm your GitPulse subscription',
      html: this._buildConfirmationEmailHtml(confirmUrl),
    });
  }

  /**
   * Розсилка сповіщень про нові релізи
   */
  ...
  async _sendEmail(to, owner, repo, newTag, unsubscribeToken) {
    const repoUrl = `https://github.com/${owner}/${repo}`;
    const releaseUrl = `${repoUrl}/releases/tag/${newTag}`;
    const unsubscribeUrl = `${process.env.APP_URL || 'http://localhost:3005'}/api/unsubscribe/${unsubscribeToken}`;

    await mailClient.send({
      to: to,
      subject: `🚀 New release: ${owner}/${repo} — ${newTag}`,
      html: this._buildReleaseEmailHtml(owner, repo, newTag, repoUrl, releaseUrl, unsubscribeUrl),
    });
  }

  _buildConfirmationEmailHtml(confirmUrl) {
    return `
<!DOCTYPE html>
<html>
<body style="margin:0; padding:0; background-color:#0d1117; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif;">
  ...
  _buildReleaseEmailHtml(owner, repo, newTag, repoUrl, releaseUrl, unsubscribeUrl) {
    return `
<!DOCTYPE html>
<html>
<body style="margin:0; padding:0; background-color:#0d1117; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif;">
